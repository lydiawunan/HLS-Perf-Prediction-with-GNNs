...
class Net(torch.nn.Module):
    def __init__(self, num_tasks=1, num_layer=4, emb_dim=300, drop_ratio = 0.5, JK = "sum", residual = False, graph_pooling = "sum", fast=0):
        ...
        for i in range(num_layer):
            if fast==1:
                conv=FastRGCNConv(emb_dim, emb_dim, 4,num_bases=30,aggr='add') # add or mean
            else:
                conv=RGCNConv(emb_dim, emb_dim, 4,num_bases=30,aggr='add') # add or mean
    ...
    def forward(self, batched_data):
        ...
        for layer in range(self.num_layer):
            ...
            h = self.convs[layer](h_list[layer], edge_index, edge_attr[:,0])
