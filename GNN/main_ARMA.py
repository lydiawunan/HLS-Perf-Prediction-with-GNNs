...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='ARMA model')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loss<=np.min(np.array(test_curve)):
            PATH='model/'+args.dataset + '_arma_layer_'+ str(args.num_layer)+'_model.pt'
    ...
    f = open(args.dataset + '_arma_layer_'+str(args.num_layer)+'.json', 'w')
