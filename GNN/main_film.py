...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='FiLM model')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loss<=np.min(np.array(test_curve)):
            PATH='model/1_'+args.dataset + '_film_layer_'+ str(args.num_layer)+'_model.pt'
    ...
    f = open('1_'+args.dataset + '_film_layer_'+str(args.num_layer)+'.json', 'w')
