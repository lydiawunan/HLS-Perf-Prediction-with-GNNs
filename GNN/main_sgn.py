...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='SGN model')
    ...
    parser.add_argument('--dataset', type=str, default="dsp_lut",
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loss<=np.min(np.array(test_curve)):
            PATH='model/'+args.dataset + '_sgn_layer_'+ str(args.num_layer)+'_model.pt'
    ...
    f = open(args.dataset + '_sgn_layer_'+str(args.num_layer)+'.json', 'w')
