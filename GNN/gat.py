...
class Net(torch.nn.Module):
    def __init__(self, heads=8, num_tasks=1, num_layer=4, emb_dim=300, drop_ratio = 0.5, JK = "last", residual = False, graph_pooling = "sum"):
        ...
        conv=GATConv(emb_dim, emb_dim, heads=heads)
        self.convs.append(conv)
        for i in range(num_layer-2):
            conv=GATConv(heads * emb_dim, emb_dim, heads=heads)
            self.convs.append(conv)
            #self.batch_norms.append(BatchNorm(emb_dim))
        conv=GATConv(heads*emb_dim, emb_dim, heads=1, concat=False)
        self.convs.append(conv)
