...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='GGNN model')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loss<=np.min(np.array(test_curve)):
            PATH='model/'+args.dataset + '_ggnn_layer_'+ str(args.num_layer)+'_model.pt'
    ...
    f = open(args.dataset + '_ggnn_layer_'+str(args.num_layer)+'.json', 'w')
