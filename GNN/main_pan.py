...
def main():
    # Training settings
    parser = argparse.ArgumentParser(description='PAN model')
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if test_loss<=np.min(np.array(test_curve)):
            PATH='model/'+args.dataset + '_pan_layer_'+ str(args.num_layer)+'_model.pt'
    ...
    f = open(args.dataset + '_pan_layer_'+str(args.num_layer)+'.json', 'w')
