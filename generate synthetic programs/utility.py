...
exit
    ...
    p = subprocess.Popen(['rm', '-rf', dic+'project_tmp'], stderr=subprocess.STDOUT)
    p = subprocess.Popen(['vitis_hls', '-f', script_name], stderr=subprocess.STDOUT,cwd=dic)
    t = 0
    while( t < 2000 ):
        time.sleep(5)
        if p.poll() != None:
    ...
    for adb_file in glob.glob(xml_file_path + '*.adb'):
        ...
        print(func_name)
        p = subprocess.Popen(['cp', adb_file, './' + dic+case_name + '_' + func_name + '.adb'], stderr=subprocess.STDOUT)
